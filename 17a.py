...
img = ''
runcode(code_str, input_handler, output_handler)

scaffolds = [[char == '#' for char in line] for line in img.rstrip().splitlines()]

sum = 0
for i, line in enumerate(scaffolds):
  if i == 0 or i == len(scaffolds) - 1:
    continue
  for j, scaffold in enumerate(line):
    if j == 0 or j == len(line) - 1:
      continue
    if scaffold and scaffolds[i - 1][j] and scaffolds[i + 1][j] and scaffolds[i][j - 1] and scaffolds[i][j + 1]:
      print(f'i {i} * j {j} = {i*j}')
      sum += i*j
