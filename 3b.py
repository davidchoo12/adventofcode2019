  ...
  for l in w1:
    if (l[0] == 'U'):
      ...
      for i in range(y, y+l[1]):
        w1xy.setdefault(x, {}).setdefault(i, step)
        step += 1
      y += l[1] - 1
    elif (l[0] == 'D'):
      y -= 1
      ...
      for i in range(x, x+l[1]):
        w1xy.setdefault(i, {}).setdefault(y, step)
        step += 1
      x += l[1] - 1
    elif (l[0] == 'L'):
      x -= 1
...
for i in intersects:
  for j in intersects[i]:
    shortest = min(w1xy[i][j] + w2xy[i][j], shortest)
