...
def getXY(w):
  ...
  for l in w1:
    if (l[0] == 'U'):
      y += 1
      w1xy.setdefault(x, []).extend(range(y, y+l[1]))
      y += l[1] - 1
    elif (l[0] == 'D'):
      y -= 1
      w1xy.setdefault(x, []).extend(range(y, y-l[1], -1))
      y -= l[1] - 1
    elif (l[0] == 'R'):
      x += 1
      for i in range(x, x+l[1]):
        w1xy.setdefault(i, []).append(y)
      x += l[1] - 1
    elif (l[0] == 'L'):
      x -= 1
...
# display graph of wires
# for i in range(100, -100, -1): # y
#   print('{:03}'.format(i), end=':')
#   for j in range(-1, 300): # x
#     if j in w1xy and i in w1xy[j]:
#       if j in w2xy and i in w2xy[j]:
#         print('X', end='')
#       else:
#         print('1', end='')
#     elif j in w2xy and i in w2xy[j]:
#       print('2', end='')
#     else:
#       print('.', end='')
#   print()
...
# find intersects
w2 = list(map(lambda e: [e[0], int(e[1:])], w2.split(',')))
x = 0
y = 0
intersects = {}
for l in w2:
  if (l[0] == 'U'):
    ...
    for i in range(y, y+l[1]):
      if x in w1xy and i in w1xy[x]:
        intersects.setdefault(x, []).append(i)
    y += l[1] - 1
  elif (l[0] == 'D'):
    y -= 1
    ...
    x += 1
    for i in range(x, x+l[1]):
      if i in w1xy and y in w1xy[i]:
        intersects.setdefault(i, []).append(y)
    x += l[1] - 1
  elif (l[0] == 'L'):
    x -= 1
    for i in range(x, x-l[1], -1):
...
# find shortest intersect
shortest = 100000000
for i in intersects:
  for j in intersects[i]:
    shortest = min(abs(i) + abs(j), shortest)
